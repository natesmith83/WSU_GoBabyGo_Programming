...
int line_sensor = 5;                    //line hunter sensor
...
void loop()
{
  ...
  do{
    if(HB > 900 && HL > 900 && HR > 900 && REV > 900) { 
  ...
  else if(REV > 950)                   //for right movement
  {
  ...
  while (now - time_last_used < DELAY_MILLISECONDS); //to stop
  {
    _mStop();
  }
}
